        ...
        ? filtered.map((country,i) => 
            <div data-value={country.name} className="country-card" key={i}>
                <div className="flag-image">
                  <img src={`https://countries.petethompson.net/data/flags/${country.alpha3Code.toLowerCase()}.svg`} alt={`flag of ${country.name}`}/>
                  {/* <img src={`https://countries.petethompson.net/data/flags/afg.svg`} alt={`flag of ${country.name}`}/> */}
                </div>
              <Link to={`/details`}>
